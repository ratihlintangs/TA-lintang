...
# Smoothing
df['PS_smooth'] = df['PS'].rolling(window=3, center=True).mean()
df = df.dropna(subset=['PS_smooth'])

# Outlier removal
df['zscore'] = zscore(df['PS_smooth'])
df = df[df['zscore'].abs() < 3]

# Fitur lag & musiman
df['lag_7'] = df['PS_smooth'].shift(7)
df['lag_14'] = df['PS_smooth'].shift(14)
df['lag_30'] = df['PS_smooth'].shift(30)
...
def predict_model(df, feature_names, label):
    ...
    lag_values = list(df['PS_smooth'].values[-30:])
...
def predict_with_rolling(df, feature_names, label):
    ...
    for i in range(1, 8):
        ...
        temp_df['lag_7'] = temp_df['PS_smooth'].shift(7)
        temp_df['lag_14'] = temp_df['PS_smooth'].shift(14)
        temp_df['lag_30'] = temp_df['PS_smooth'].shift(30)
        ...
        row = {
            'lag_7': df_copy['PS_smooth'].iloc[-7],
            'lag_14': df_copy['PS_smooth'].iloc[-14],
            'lag_30': df_copy['PS_smooth'].iloc[-30],
            'sin_day': sin_day,
            'cos_day': cos_day,
            'dayofweek': dow
        }
        ...
        df_copy = pd.concat([df_copy, pd.DataFrame({'tanggal': [next_date], 'PS_smooth': [y1]})], ignore_index=True)
