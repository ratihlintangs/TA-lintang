...
actual = np.array([25.3, 24.52, 25.26, 25.08, 24.46, 24.38, 24.32])
...
results_A = predict_model(df, ['dayofyear'], "Kombinasi A")

print("\n========== KOMBINASI B: Lag ==========")
results_B = predict_model(df, ['lag_7', 'lag_14', 'lag_30'], "Kombinasi B")

print("\n========== KOMBINASI C: Lag + Musiman ==========")
results_C = predict_model(df, ['lag_7', 'lag_14', 'lag_30', 'sin_day', 'cos_day'], "Kombinasi C")

print("\n========== KOMBINASI D: Lag + Musiman + Rolling ==========")
results_D = predict_with_rolling(df, ['lag_7', 'lag_14', 'lag_30', 'sin_day', 'cos_day'], "Kombinasi D")

# === Gabungkan dan Simpan ke CSV ===
df_all_prediksi = pd.concat([results_A, results_B, results_C, results_D], ignore_index=True)
df_all_prediksi.to_csv("hasil_prediksi_semua_kombinasi.csv", index=False)

# === Simpan Evaluasi ke CSV ===
df_evaluasi = pd.DataFrame(evaluasi_list)
df_evaluasi.to_csv("evaluasi_per_minggu.csv", index=False)

print("\n✅ File 'hasil_prediksi_semua_kombinasi.csv' dan 'evaluasi_per_minggu.csv' berhasil disimpan.")
...
plt.title("Prediksi T2M 1–7 Juni 2025 (Kombinasi D)")
plt.xlabel("Tanggal")
plt.ylabel("T2M (°C)")
plt.legend()
plt.grid(True)
