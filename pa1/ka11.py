...
# Smoothing
df['WS2M_smooth'] = df['WS2M'].rolling(window=3, center=True).mean()
df = df.dropna(subset=['WS2M_smooth'])

# Outlier removal
df['zscore'] = zscore(df['WS2M_smooth'])
df = df[df['zscore'].abs() < 3]

# Fitur lag & musiman
df['lag_7'] = df['WS2M_smooth'].shift(7)
df['lag_14'] = df['WS2M_smooth'].shift(14)
df['lag_30'] = df['WS2M_smooth'].shift(30)
...
def predict_model(df, feature_names, label):
    ...
    lag_values = list(df['WS2M_smooth'].values[-30:])
...
def predict_with_rolling(df, feature_names, label):
    ...
    for i in range(1, 8):
        ...
        temp_df['lag_7'] = temp_df['WS2M_smooth'].shift(7)
        temp_df['lag_14'] = temp_df['WS2M_smooth'].shift(14)
        temp_df['lag_30'] = temp_df['WS2M_smooth'].shift(30)
        ...
        row = {
            'lag_7': df_copy['WS2M_smooth'].iloc[-7],
            'lag_14': df_copy['WS2M_smooth'].iloc[-14],
            'lag_30': df_copy['WS2M_smooth'].iloc[-30],
            'sin_day': sin_day,
            'cos_day': cos_day,
            'dayofweek': dow
        }
        ...
        df_copy = pd.concat([df_copy, pd.DataFrame({'tanggal': [next_date], 'WS2M_smooth': [y1]})], ignore_index=True)
...
df_evaluasi.to_csv("evaluasi_per_minggu11.csv", index=False)
print("\n✅ File 'evaluasi_per_minggu11.csv' berhasil disimpan.")
