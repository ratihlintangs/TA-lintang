...
df['lag_7'] = df['RH2M'].shift(7)
df['lag_14'] = df['RH2M'].shift(14)
df['lag_30'] = df['RH2M'].shift(30)
...
def predict_model(df, feature_names, label):
    ...
    lag_values = list(df['RH2M'].values[-30:])
...
def predict_with_rolling(df, feature_names, label):
    ...
    for i in range(1, 8):
        ...
        temp_df['lag_7'] = temp_df['RH2M'].shift(7)
        temp_df['lag_14'] = temp_df['RH2M'].shift(14)
        temp_df['lag_30'] = temp_df['RH2M'].shift(30)
        ...
        lag_7 = df_copy['RH2M'].iloc[-7]
        lag_14 = df_copy['RH2M'].iloc[-14]
        lag_30 = df_copy['RH2M'].iloc[-30]
        ...
        df_copy = pd.concat([df_copy, pd.DataFrame({'tanggal': [next_date], 'RH2M': [y1]})], ignore_index=True)
...
plt.tight_layout()
plt.show()
# === Simpan Semua Hasil Prediksi RH2M ke Satu File CSV ===
...
df_all_prediksi.to_csv("hasil_prediksi_RH2M.csv", index=False)
print("\n✅ File 'hasil_prediksi_RH2M.csv' berhasil disimpan.")
