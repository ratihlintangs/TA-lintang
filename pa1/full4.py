...
df['PS'] = df['PS'].replace('-', np.nan)
df['PS'] = pd.to_numeric(df['PS'], errors='coerce')
df = df.dropna(subset=['PS'])

# Tambah fitur lag dan musiman
df['lag_7'] = df['PS'].shift(7)
df['lag_14'] = df['PS'].shift(14)
df['lag_30'] = df['PS'].shift(30)
...
def predict_model(df, feature_names, label):
    ...
    lag_values = list(df['PS'].values[-30:])
...
def predict_with_rolling(df, feature_names, label):
    ...
    for i in range(1, 8):
        ...
        temp_df['lag_7'] = temp_df['PS'].shift(7)
        temp_df['lag_14'] = temp_df['PS'].shift(14)
        temp_df['lag_30'] = temp_df['PS'].shift(30)
        ...
        lag_7 = df_copy['PS'].iloc[-7]
        lag_14 = df_copy['PS'].iloc[-14]
        lag_30 = df_copy['PS'].iloc[-30]
        ...
        df_copy = pd.concat([df_copy, pd.DataFrame({'tanggal': [next_date], 'PS': [y1]})], ignore_index=True)
...
plt.title("Prediksi PS 1–7 Juni 2025")
plt.xlabel("Tanggal")
plt.ylabel("PS (°C)")
plt.legend()
plt.grid(True)
...
plt.tight_layout()
plt.show()
# === Simpan Semua Hasil Prediksi PS ke Satu File CSV ===
results_A['Kombinasi'] = 'Kombinasi A'
results_B['Kombinasi'] = 'Kombinasi B'
results_C['Kombinasi'] = 'Kombinasi C'
results_D['Kombinasi'] = 'Kombinasi D'

df_all_prediksi = pd.concat([results_A, results_B, results_C, results_D], ignore_index=True)
df_all_prediksi.to_csv("hasil_prediksi_PS.csv", index=False)
print("\n✅ File 'hasil_prediksi_PS.csv' berhasil disimpan.")
