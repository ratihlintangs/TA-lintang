...
for i in range(1, 8):
    ...
    # Simpan hasil (pembulatan 1 angka di belakang koma)
    preds_linear.append(round(pred_lin, 1))
    preds_poly.append(round(pred_pol, 1))
    preds_nonlinear.append(round(pred_non, 1))
    future_dates.append(next_date)

    # Update lag list: bisa pilih model mana sebagai baseline update
    lag_values.append(pred_lin)  # gunakan pred_lin untuk menjaga konsistensi input rekursif

# === 5. DATA AKTUAL ===
actual = np.array([27.9, 28.7, 29.4, 29.6, 30.1, 28.8, 29.0])
...
plt.title("Perbandingan Suhu Aktual vs Prediksi (1–7 Juni 2025)")
plt.xlabel("Tanggal")
